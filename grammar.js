...
const non_tab_control_char = /[^\x00-\x08\x0A-\x1F\x7F]/;
const basic_string_char = /[^\x00-\x08\x0A-\x1F\x7F"\\]/;
const literal_string_char = /[^\x00-\x08\x0A-\x1F\x7F']/;
const newline = /\r?\n/;
...
module.exports = grammar({
  ...
  rules: {
    ...
    comment: $ =>
      token(prec(-1, seq(
        "#",
        repeat(non_tab_control_char),
      ))),

    table: $ =>
      seq(
        "[",
        field("table_name", choice($.dotted_key, $._key)),
        "]",
        $._line_ending_or_eof,
        repeat(choice($.pair, newline)),
      ),
    ...
    _inline_pair: $ => seq(field('key', choice($.dotted_key, $._key)), "=", field('value', $._inline_value)),
